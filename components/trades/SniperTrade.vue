...
import {OrderDirection, SwitchState, TradeErrorCode, TradeStatus} from "@/utils/constant";
...
import {SniperOrder} from "~/models/SniperOrder";
...
interface SniperTradeProps {
  type: OrderDetailType
}
const route = useRoute();
const routePath = route.path
let source = routeSource(routePath);
const props = defineProps<SniperTradeProps>()
let airdropTokens: { [key: string]: string } = {};

let orderList: Array<SniperOrder> = [];
...
const dataList = ref<SniperOrder[]>([]);
...
const parseSniperOrderList = (result: Result) => {
  if (result.code == 0) {
    let data = result.data;
    if (data) {
      let tokens = data.orders;
...
const forceDemo = () => {}
const loadSniperOrderList = () => {
  loading.value = true;
  setTimeout(async () => {
    try {
      let result: Result = await $api.snipOrderList();
      parseSniperOrderList(result);
    } catch (e: any) {
    } finally {
      loading.value = false;
    }
...
const getFilterData = (orderList: Array<SniperOrder>, term: string, filterComplete: SwitchState) => {
  return orderList.filter((item) => {
    ...
    if (filterComplete == SwitchState.on) {
      ok4 = item.status == TradeStatus.Success || item.statusMessage == TradeErrorCode.TxSendSuccess;
    }
    return (ok1 || ok2 || ok3) && ok4;
  });
}
const sortTradeDetail = (a: SnipOrder, b: SnipOrder) => {
...
const openTaskName = (address: string) => {
  ...
  url.pathname = `trades`
  url.searchParams.set("t", "3")
  url.hash = address.replace("0x", "");
  window.open(url.href, "_self")
}
const orderTradeShareDisabledState = (order: SniperOrder) => {
  return order.hash.length == 0 || order.direction != OrderDirection.Sell;
}
const getMenuState = (order: SniperOrder) => {
  return {
...
const openOrderStatistics = (order: SniperOrder) => {
  tradeDetailVisible.value = true;
  tradeDetailPair.value = order.pair;
}

const openOrderTokenSell = (order: SniperOrder) => {
...
const orderTooltipMessage = (order: SniperOrder) => tradeErrorTipMessage(store.chainId, order.param1, order.param2, order.statusMessage);
const orderTooltipStatus = (order: SniperOrder) => {
  let text = tradeErrorTipMessage(store.chainId, order.param1, order.param2, order.statusMessage);
  return text == null || text.length == 0;
}
const orderStatusTip = (order: SniperOrder) => {
...
const onOrderMenu = (key: TradeMenuKeys, order: SniperOrder) => {
...
<template>
  <div class="column gap-24">
    <div class="row flex-start" v-if="!loading">
      <Switch :state="hideCompleted" :text="$t('Trades001',{name:$t('Trades005')})"
